...
class StockApi(db.Model):
    """
    股票操作表（动态路由）
    """
    __tablename__ = 'stock_api'

    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()), unique=True,
                   nullable=False, comment="哈希自动生成id")
    total_assets = db.Column(db.String(36), comment="合计资产")
...
class TransactionList(db.Model):
    ...
    subtotal_price = db.Column(db.Float, nullable=False, comment='小计买入价格')
